...
默认每个提交由 Claude 自审（显式标记）。设 `EVERCODE_CODEX=1` 可启用 Codex 双评审——每次提交再经第二个、用独立提示词的 LLM 独立评审。不启用时失去双 LLM 保障，但工作循环不停。
...
每个任务的流程：Claude 规划 → Claude 实现 → 代码评审（默认 Claude 自审；`EVERCODE_CODEX=1` 时 Codex 循环评审直到干净、10 轮后硬停并回滚）→ 测试必须通过 → 文件门控提交。预提交关卡会拒绝暂存那些 `code-review.txt` 缺失、为空、或缺少对应评审 header 的任务。
...
启用 Codex（`EVERCODE_CODEX=1`）时，代理可在 Codex 同意后提前结束班次（双共识）。默认不启用时，班次运行到 8 小时硬上限——代理不能自判完工。
...
- **OpenAI [Codex CLI](https://github.com/openai/codex)**（可选增强）。设 `EVERCODE_CODEX=1` 启用双 LLM 评审循环和“基于共识收尾”。默认 Claude 自审，交接文档标出未经 Codex 的任务，班次运行到 8 小时上限。
